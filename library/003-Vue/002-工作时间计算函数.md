...
dateDiff (fromTime, toTime) {
  if (_.isEmpty(fromTime) || _.isEmpty(toTime)) return 0 //当参数不齐时返回0
  let workDuration = [[8*3600*1000, 11.75*3600*1000], [13*3600*1000, 17*3600*1000]] //设定上班时间段
  fromTime = + new Date(fromTime) //将时间字符串转化为时间戳
  ...
  let splitTimePart = []
  let fromTimeAtDay = new Date(fromTime).getHours()*3600*1000 + new Date(fromTime).getMinutes()*60*1000 + new Date(fromTime).getSeconds()*1000 //排除日期后的起始时间点
  let toTimeAtDay = new Date(toTime).getHours()*3600*1000 + new Date(toTime).getMinutes()*60*1000 + new Date(toTime).getSeconds()*1000
  let fullDayDiff = (preDiffTime - toTimeAtDay - (oneDay - fromTimeAtDay)) / oneDay //计算两点之间的完整天数
  let usedTime = 0
